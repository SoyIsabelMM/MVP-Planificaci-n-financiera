...
export default function CommentBox() {
  return (
    <div className="w-[90%] rounded-lg border-none bg-white shadow-md overflow-hidden">
      <div className="flex flex-wrap justify-center items-center gap-4 mt-10 mx-auto max-w-full p-5">
        {items.map((item) => (
          <div
            key={item.id}
            className="bg-black w-[50% d] h-[130px] p-5 rounded-lg flex flex-col items-center justify-center gap-2"
          >
            {item.icon && (
              <Image
                src={item.icon}
